...
output_filename = '../glovelinad.gif'
...
def deformationFrame(data):
    global flex
    flex = (min(max(0, -int((data[0] - 200) * 3.3)), 90),
            min(max(0, int((data[4] - 74) * 1.2)), 90),
            min(max(0, int((data[3] - 130) * 1.5)), 90),
            min(max(0, int((data[2] - 45) * 1.0)), 90),
            min(max(0, int((data[1] - 130) * 1.5)), 90))
